...
DeviceRegEx = re.compile(
    r"\\\\\?\\(\w+)#VID_([0-9a-fA-F]+)\&PID_([0-9a-fA-F]+)#", re.IGNORECASE)
...
class HIDThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            (errno, function, strerror) = xxx_todo_changeme1.args
        ...
        try:
            ...
            while not self.abort:
                ...
                try:
                    ...
                    (errno, function, strerror) = xxx_todo_changeme.args
                    # device got disconnected so set status to waiting
                    self.abort = True
                    if errno == 1167:
    ...
    def SetFeature(self, buffer):
        if self.handle:
            ...
            result = ctypes.windll.hid.HidD_SetFeature(
                int(self.handle),
                ctypes.create_string_buffer(buffer), bufferLength)
            if not result:
...
class DeviceDescription():
    def __init__(self, devicePath, vendorId, vendorString, productId,
                 productString, versionNumber):
        self.devicePath = devicePath
...
def GetDeviceDescriptions():
    ...
    # enumerate devices
    i = 0
    while setupapiDLL.SetupDiEnumDeviceInterfaces(hinfo, None,
                                                  byref(g), i,
                                                  byref(interfaceInfo)):
        i += 1
        ...
        setupapiDLL.SetupDiGetDeviceInterfaceDetailA(
            hinfo,
            byref(interfaceInfo),
            byref(interfaceDetailData), requiredSize,
            pointer(requiredSize), None)
        ...
        if not result or len(infoStr.value) == 0:
            ...
            productString, regType = _winreg.QueryValueEx(regHandle,
                                                          "DeviceDesc")
    ...
    for item in deviceList:
        if noOtherPort:
            ...
            if validVendorId and validProductId and validVersionNumber:
                if item.devicePath == devicePath or (useDeviceIndex and
                                                     deviceIndex == found):
                    # found right device
...
def IsDeviceName(deviceNameList, vid, pid):
    ...
    return deviceClass == "HID" and pid == int(pidStr, 16) and vid == int(
        vidStr, 16)
...
class SP_DEVICE_INTERFACE_DATA(Structure):
    _fields_ = [("cbSize", c_ulong), ("InterfaceClassGuid", GUID),
                ("Flags", c_ulong), ("Reserved", POINTER(ULONG))]


class SP_DEVICE_INTERFACE_DETAIL_DATA_A(Structure):
    _fields_ = [("cbSize", c_ulong), ("DevicePath", c_char * 255)]


class HIDD_ATTRIBUTES(Structure):
    _fields_ = [("cbSize", c_ulong), ("VendorID", c_ushort),
                ("ProductID", c_ushort), ("VersionNumber", c_ushort)]


class HIDP_CAPS(Structure):
    _fields_ = [("Usage", c_ushort), ("UsagePage", c_ushort),
...
class HIDP_CAPS_UNION(Union):
    class HIDP_BUTTON_CAPS_RANGE(Structure):
        _fields_ = [("UsageMin", c_ushort), ("UsageMax", c_ushort),
    ...
    class HIDP_BUTTON_CAPS_NOT_RANGE(Structure):
        _fields_ = [("Usage", c_ushort), ("Reserved1", c_ushort),
                    ("StringIndex", c_ushort), ("Reserved2", c_ushort),
                    ("DesignatorIndex", c_ushort), ("Reserved3", c_ushort),
                    ("DataIndex", c_ushort), ("Reserved4", c_ushort)]

    _fields_ = [("Range", HIDP_BUTTON_CAPS_RANGE),
                ("NotRange", HIDP_BUTTON_CAPS_NOT_RANGE)]


class HIDP_BUTTON_CAPS(Structure):
    _fields_ = [("UsagePage", c_ushort), ("ReportID", c_char),
                ("IsAlias", BOOLEAN), ("BitField", c_ushort),
                ("LinkCollection", c_ushort), ("LinkUsage", c_ushort),
                ("LinkUsagePage", c_ushort), ("IsRange", BOOLEAN),
                ("IsStringRange", BOOLEAN), ("IsDesignatorRange", BOOLEAN),
                ("IsAbsolute", BOOLEAN), ("Reserved", c_ulong * 10),
                ("Info", HIDP_CAPS_UNION)]


class HIDP_VALUE_CAPS(Structure):
    _fields_ = [
        ("UsagePage", c_ushort), ("ReportID", c_char), ("IsAlias", BOOLEAN),
        ("BitField", c_ushort), ("LinkCollection", c_ushort),
        ("LinkUsage", c_ushort), ("LinkUsagePage", c_ushort),
        ("IsRange", BOOLEAN), ("IsStringRange", BOOLEAN),
        ("IsDesignatorRange", BOOLEAN), ("IsAbsolute", BOOLEAN),
        ("HasNull", BOOLEAN), ("Reserved", c_char), ("BitSize", c_ushort),
        ("ReportCount", c_ushort), ("Reserved2", c_ushort * 5),
        ("UnitsExp", c_ulong), ("Units", c_ulong), ("LogicalMin", c_long),
        ("LogicalMax", c_long), ("PhysicalMin", c_long),
        ("PhysicalMax", c_long), ("Info", HIDP_CAPS_UNION)
    ]
...
class HIDP_DATA(Structure):
    ...
    _fields_ = [("DataIndex", c_ushort), ("Reserved", c_ushort),
