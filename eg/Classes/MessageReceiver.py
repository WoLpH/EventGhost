...
class MessageReceiver(eg.ThreadWorker):
    ...
    def __init__(self, windowName):
        ...
        self.messageProcs = {WM_SIZE: [self.WmSizeHandler], }
    ...
    def Setup(self):
        ...
        self.hwnd = CreateWindowEx(
            0, self.wndclass.lpszClassName, self.windowName,
            WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
            CW_USEDEFAULT, 0, 0, self.wndclass.hInstance, None)
        if not self.hwnd:
            raise WinError()
    ...
    def Stop(self):
        ...
        if not UnregisterClass(
                cast(self.classAtom, LPCTSTR), GetModuleHandle(None)):
            raise WinError()
